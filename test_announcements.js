...
const testAnnouncements = async () => {
  try {
    ...
    const rootResponse = await axios.get('https://itdlhsms-production.up.railway.app/');
    console.log('Root endpoint response:', rootResponse.data);
    
    // Test announcements endpoint (this should fail with 401 since no auth)
    try {
      const announcementsResponse = await axios.get('https://itdlhsms-production.up.railway.app/api/announcements/admin/all');
