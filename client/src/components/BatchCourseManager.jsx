...
export default function BatchCourseManager() {
  ...
  const fetchData = async () => {
    try {
      const [batchesRes, coursesRes, structuresRes, batchCoursesRes] = await Promise.all([
        ...
        axios.get('http://localhost:5000/api/attendance/batch-courses')
      ]);
  ...
  const fetchClasses = async (courseId) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/attendance/courses/${courseId}/classes`);
      setClasses(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/attendance/batch-course', payload);
  ...
  const handleDeactivate = async (batchCourseId) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/attendance/batch-course/${batchCourseId}/deactivate`);
