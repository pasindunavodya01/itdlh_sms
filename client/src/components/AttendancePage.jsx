...
export default function AttendanceManager() {
  ...
  const fetchData = async () => {
    try {
      const [bcRes, sessionsRes, summaryRes, supportRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/attendance/batch-course/${batchCourseId}`),
        axios.get(`http://localhost:5000/api/attendance/sessions?batch_course_id=${batchCourseId}`),
        axios.get(`http://localhost:5000/api/attendance/attendance-summary/${batchCourseId}`),
        axios.get(`http://localhost:5000/api/attendance/students-needing-support/${batchCourseId}`)
      ]);
  ...
  const fetchStudents = async (sessionId) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/attendance/session/${sessionId}/students`);
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/attendance/session', payload);
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/attendance/session/${selectedSession.session_id}/attendance`,
        { attendance: payload }
      );
